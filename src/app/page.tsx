...
function isTodayInIndia(value: string): boolean {
  const time = new Date(value).getTime();
  return Number.isFinite(time) && getIndiaDateKey(new Date(time)) === getIndiaDateKey();
}
...
export default function Dashboard() {
  ...
  // Only restore entries whose LAST SEEN timestamp belongs to today's IST date.
  // This protects against stale/corrupted localStorage data from earlier days.
  useEffect(() => {
    try {
      ...
      if (raw) {
        ...
        if (Array.isArray(parsed)) {
          const todayOnly = parsed.filter((item) => isTodayInIndia(item.lastSeen || item.triggeredAt));
          setDailyResults(todayOnly);
          if (todayOnly.length !== parsed.length) window.localStorage.setItem(storageKey, JSON.stringify(todayOnly));
        }
      }
    } catch (storageError) {
  ...
  useEffect(() => {
    if (!hydrated) return;
    try {
      window.localStorage.setItem(storageKey, JSON.stringify(dailyResults.filter((item) => isTodayInIndia(item.lastSeen || item.triggeredAt))));
    } catch (storageError) {
  ...
  const performScan = useCallback(async () => {
    ...
    try {
      ...
      const incoming = (data.results || []).filter((item) => isTodayInIndia(item.triggeredAt || seenAt));
      const todayExisting = dailyResults.filter((item) => isTodayInIndia(item.lastSeen || item.triggeredAt));
      const previousSymbols = new Set(todayExisting.map((item) => item.symbol));
      const bySymbol = new Map(todayExisting.map((item) => [item.symbol, item]));
      ...
      for (const result of incoming) {
        ...
        bySymbol.set(result.symbol, {
          ...existing,
          ...result,
          firstSeen: existing?.firstSeen && isTodayInIndia(existing.firstSeen) ? existing.firstSeen : (result.triggeredAt || seenAt),
          lastSeen: seenAt,
        });
      }

      const merged = [...bySymbol.values()].filter((item) => isTodayInIndia(item.lastSeen || item.triggeredAt));
  ...
  const tabButton = (value: Tab, label: string, count: number, icon: string, activeClass: string) => (
    <button onClick={() => setTab(value)} className={`flex items-center justify-center gap-2 rounded-xl border px-4 py-3 text-sm font-semibold transition ${tab === value ? activeClass : 'border-gray-800 bg-gray-900/70 text-gray-300 hover:border-gray-700 hover:bg-gray-800'}`}>
    ...
    <header className="sticky top-0 z-20 border-b border-gray-800 bg-gray-900/95 backdrop-blur">
      <div className="container mx-auto px-4 py-4">
        <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div><h1 className="text-2xl font-bold text-blue-400 md:text-3xl">📊 Real-Time Stock Scanner</h1><p className="mt-1 text-xs text-gray-500">NSE F&O • Upstox • 09:15–15:30 IST • Daily signal memory enabled</p></div>
          <div className={`rounded-full px-4 py-2 text-sm font-semibold ${marketOpen === true ? 'bg-green-900/30 text-green-400' : marketOpen === false ? 'bg-gray-800 text-gray-400' : 'bg-blue-900/30 text-blue-400'}`}>{marketOpen === true ? '● MARKET OPEN' : marketOpen === false ? '● MARKET CLOSED' : '● CHECKING MARKET'}</div>
        </div>
